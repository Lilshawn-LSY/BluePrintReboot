...
- [x] v1.6.1 web Reader drafts are scoped to stable Paper identity and browser-persisted. Route navigation, unmount/remount, browser refresh, API unavailability/restart, command failure, and revision conflict do not discard local Paper Note work. Streamlit retains its separate historical session-state policy.
- [x] The web task-facing save state is limited to Saved, Unsaved changes, Saving..., Save failed, Changed elsewhere, and Offline. HTTP/revision terminology is secondary rather than the primary UI label.
- [x] A successful write clears only the exact confirmed request snapshot. A later edit made during that request remains locally persisted and Unsaved changes; duplicate Save actions cannot create concurrent writes.
- [x] Conflict state keeps the local draft and latest remote value separately and exposes keep-my-draft, use-latest-server-value, and practical local-versus-latest review actions.
