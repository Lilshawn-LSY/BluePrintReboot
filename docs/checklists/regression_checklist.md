...
- [x] Keep PR #5 merge evidence, PR-head CI, and unverified post-merge `main` CI structurally separate.
- [x] Confirm rendering is deterministic and idempotent, and reject a deliberately stale generated status.
...
- [x] Regress the development setup/cleanup/setup sequence with controlled promises: discarded A owns and cancels only A, active B renders page 1, and late A cleanup cannot clear B's canvas or publish stale state.
- [x] Confirm destroy waits for asynchronous task creation, observes pending `loadingTask.promise` cancellation, retry starts one authoritative operation, and a destroyed unmounted controller cannot restart.
- [x] Confirm the pinned official PDF.js distribution contains upstream PR #20503's worker-cancellation fix while the worker remains enabled, local, and emitted by the production build.
