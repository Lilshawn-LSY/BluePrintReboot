...
def test_corrupt_projects_file_raises_typed_error_and_preserves_file() -> None:
    base_dir = make_workspace("projects-corrupt")
    ...
    with pytest.raises(CorruptJsonError) as error:
        list_projects(base_dir)

    assert error.value.path == path
    assert "Projects file is invalid JSON" in error.value.summary
