...
def test_schema_three_manifest_is_the_current_release_authority() -> None:
    manifest = read_manifest()
    ...
    assert manifest["source_control"]["pull_request"]["number"] == 4
    assert manifest["source_control"]["pull_request"]["state"] == "MERGED"
    assert manifest["source_control"]["tag"]["status"] == "VERIFIED"
    assert manifest["automated_validation"]["pr_head_ci"]["run_id"] == "30145836293"
...
def test_generated_current_status_is_the_only_volatile_document_surface() -> None:
    ...
    assert "PR #4" in current_status
    assert "30145836293" in current_status
    assert "| v1.4.0 tag | VERIFIED |" in current_status
    assert "| Post-merge `main` GitHub Actions | NOT VERIFIED |" in current_status
    assert "| Reader runtime | PARTIALLY VERIFIED |" in current_status
    assert "| Streamlit regression | NOT VERIFIED |" in current_status
