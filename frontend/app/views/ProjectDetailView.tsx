...
function ProjectWorkspace({ snapshot }: { snapshot: ProjectDetail }) {
  ...
  return (
    <>
      ...
      <Section title="Linked Papers" description="Papers that inform this research question.">
        {paperLinks.length === 0 ? <EmptyState title="No linked Papers" description="Add a Paper from Manage links when it becomes useful to this project." /> : <div className="project-linked-list">{paperLinks.map((link) => <article className="project-material-row" key={link.link_id}><div className="project-material-row__identity">{link.paper ? <Link className="paper-link" href={`/papers/${encodeURIComponent(link.paper.paper_id)}`}>{link.paper.title || "Stored title unavailable"}</Link> : <strong>Linked Paper unavailable</strong>}<p>{link.paper ? `${link.paper.first_author || "Author not recorded"}${link.paper.year ? ` · ${link.paper.year}` : ""}` : "The Paper is not currently available."}</p></div><div className="project-material-row__metadata"><RelationshipLabel type={link.link_type} />{link.target_state !== "available" ? <StatusBadge tone={targetStateTone(link.target_state)}>{link.target_state.replaceAll("_", " ")}</StatusBadge> : null}</div>{!archived ? <button className="icon-button project-material-row__remove" type="button" title="Remove Paper link" aria-label={`Remove link to ${link.paper?.title || "this Paper"}`} onClick={() => removePaperLink(link.link_id, link.paper?.title || "this Paper")} disabled={linkStatus === "saving" || dirty}><Unlink size={15} /></button> : <span className="project-material-row__readonly">Read only</span>}</article>)}</div>}
      </Section>
      <Section title="Linked Note Blocks" description="Saved observations and ideas to synthesize here.">
        {noteBlockLinks.length === 0 ? <EmptyState title="No linked Note Blocks" description="Add an existing Note Block from Manage links when it supports this project." /> : <div className="project-linked-list">{noteBlockLinks.map((link) => <article className="project-material-row" key={link.link_id}><div className="project-material-row__identity">{link.note_block ? <Link className="paper-link" href={`/papers/${encodeURIComponent(link.note_block.paper_id)}/reader?noteBlock=${encodeURIComponent(link.note_block.block_id)}`}>{link.note_block.title || `${link.note_block.block_type} Note Block`}</Link> : link.target_state === "orphaned_note_block" ? <Link className="paper-link" href={`/papers/${encodeURIComponent(link.paper_id)}/reader?noteBlock=${encodeURIComponent(link.target_id)}`}>Missing linked Note Block</Link> : <strong>Linked Note Block unavailable</strong>}<p>{link.note_block ? [link.note_block.source_paper_title, link.note_block.tags.length ? link.note_block.tags.join(", ") : null, link.note_block.page && `Page ${link.note_block.page}`, link.note_block.figure && `Figure ${link.note_block.figure}`, link.note_block.text_preview].filter(Boolean).join(" · ") : "The saved Note Block is not currently available."}</p></div><div className="project-material-row__metadata"><RelationshipLabel type={link.link_type} />{link.target_state !== "available" ? <StatusBadge tone={targetStateTone(link.target_state)}>{link.target_state.replaceAll("_", " ")}</StatusBadge> : null}</div>{!archived ? <button className="icon-button project-material-row__remove" type="button" title="Remove Note Block link" aria-label={`Remove link to ${link.note_block?.title || link.note_block?.block_type || "this Note Block"}`} onClick={() => removeNoteBlockLink(link.link_id, link.note_block?.title || link.note_block?.block_type || "this Note Block")} disabled={linkStatus === "saving" || dirty}><Unlink size={15} /></button> : <span className="project-material-row__readonly">Read only</span>}</article>)}</div>}
      </Section>
      ...
      {!archived && editing ? <Section title="Edit research context" description="Changes are saved only when you choose Save.">
        <form className="project-command-panel" onSubmit={saveProject}>
          ...
          <div className="project-form-grid">
            ...
            <label className={`reader-field ${dirtyFields.includes("description") ? "reader-field--changed" : ""}`}><span>Research question / working context</span><textarea rows={5} maxLength={5000} value={editor.draft.description} onChange={(event) => updateEditor((current) => ({ ...editRevisionDraft(current, { ...current.draft, description: event.target.value }), status: "dirty", message: "" }))} /></label>
