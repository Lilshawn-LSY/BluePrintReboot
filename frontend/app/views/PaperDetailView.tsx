    ...
    <div className="page-stack page-stack--detail paper-detail-overview">
      ...
      {resource.status === "success" ? (
        <>
          ...
          <PageHeader title={resource.data.title || "Untitled paper"} description={[formatAuthorSummary(resource.data.authors, resource.data.first_author), resource.data.journal, resource.data.year].filter(Boolean).join(" · ")} actions={<div className="paper-detail-actions"><div className="badge-row"><StatusBadge>{resource.data.status}</StatusBadge>{resource.data.priority ? <StatusBadge>{resource.data.priority}</StatusBadge> : null}{resource.data.archived ? <StatusBadge tone="warning">Archived</StatusBadge> : null}{resource.data.missing_pdf ? <StatusBadge tone="danger">Missing PDF</StatusBadge> : null}</div>{!resource.data.missing_pdf && resource.data.relative_pdf_path ? <Link className="reader-action" href={`/papers/${encodeURIComponent(resource.data.paper_id)}/reader`}><BookOpen size={16} />Open Reader</Link> : <div className="paper-detail-recovery"><span className="reader-action reader-action--disabled" aria-disabled="true">Reader unavailable</span><Link className="reader-action" href="/library"><LibraryBig size={16} />Open Library Scan</Link><Link className="text-link" href="/settings/diagnostics">Review diagnostics</Link></div>}</div>} />
